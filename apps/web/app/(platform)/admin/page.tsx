...
import { getAdminOverview, listManageableSongs, setEditorPick, CatalogSong } from "@/lib/api";
...
function StatCard({
  ...
  return (
    <div className="rounded-3xl border border-borderSoft bg-white p-5 shadow-card">
...
function AdminStats({ accessToken }: { accessToken: string }) {
  ...
  if (!overview) {
    return (
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[1, 2, 3, 4].map((item) => (
          <div key={item} className="h-32 animate-pulse rounded-3xl bg-violet-100" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <StatCard label="Total songs" value={overview.totalSongs} helper="Database catalog" />
      <StatCard label="Published" value={overview.publishedSongs} helper="Live on Musichub" />
      <StatCard label="Users" value={overview.totalUsers} helper={`${overview.totalAdmins} admin`} />
      <StatCard
        label="Downloads"
        value={overview.freeDownloadsEnabled ? "Free" : "Off"}
        helper={overview.remixPaymentsEnabled ? "Remix paid" : "Remix deferred"}
      />
    </div>
  );
}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Mark real uploaded songs for the home page feature sections.
          </p>
        </div>
...
export default function AdminPage() {
  ...
  if (isLoading) {
    return <div className="h-96 animate-pulse rounded-3xl bg-violet-100" />;
  }

  if (!canManage) {
    return (
      <div className="mx-auto max-w-2xl rounded-3xl border border-borderSoft bg-white p-8 text-center shadow-card">
        ...
        <p className="mt-3 text-sm leading-6 text-slate-500">
          Sign in with the seeded admin account from your environment variables to upload,
          publish, and manage real songs.
        </p>
  ...
  return (
    <div className="space-y-8">
      <section className="rounded-3xl border border-borderSoft bg-white p-6 shadow-card sm:p-8">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="pill">Admin dashboard</p>
            <h1 className="mt-4 text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl">
              Upload and manage Musichub songs
            </h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-500">
              This dashboard is connected to the NestJS API, PostgreSQL, Prisma catalog,
              protected roles, and the real upload pipeline.
            </p>
          </div>
